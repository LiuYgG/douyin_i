...
class DarenProfiles():
    ...
    def get_daren_info(self):
        ...
        time.sleep(3)
        numbers = int(input("输入循环的次数： "))
        # 循环获取
        for i in range(numbers, len(nicke_name_elements)):
            # if i == count:
            #     break
            try:
                ...
                try:
                    ...
                    if send_message is not None:
                        ...
                        time.sleep(3)
                        # 窗口-发送邀约按钮
                        send_me = self.d.find_element(By.XPATH,
                                                      '/html/body/div[3]/div/div[2]/div/div/div[3]/div/div[2]/button[1]')
                        send_me.click()
                        # 点击取消
