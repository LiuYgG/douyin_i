...
class DarenProfiles():
    ...
    def setup_worksheet(self):
        ...
        # 设置列名
        self.worksheet.cell(row=1, column=1, value='昵称')
        self.worksheet.cell(row=1, column=2, value='联系方式')
        self.worksheet.cell(row=1, column=3, value='粉丝')
        self.worksheet.cell(row=1, column=4, value='标签')
        self.worksheet.cell(row=1, column=5, value='地址')
    ...
    def get_daren_info(self):
        ...
        for i in range(len(nicke_name_elements)):
            ...
            try:
                ...
                print(f'达人昵称：{daren_name} | 联系方式: {daren_contact} | 达人粉丝数：{daren_fans} | 达人标签：{daren_type} | 达人地址：{daren_city} |达人网址：{self.d.current_url}')
                self.data_list.append({'昵称': daren_name,
                                       '联系方式': daren_contact,
                                       '粉丝': daren_fans,
                                       '标签': daren_type,
                                       '地址': daren_city,
    ...
    def write_to_worksheet(self):
        ...
        for i, data in enumerate(self.data_list):
            self.worksheet.cell(row=i + 2, column=1, value=data['昵称'])
            self.worksheet.cell(row=i + 2, column=2, value=data['联系方式'])
            self.worksheet.cell(row=i + 2, column=3, value=data['粉丝'])
            self.worksheet.cell(row=i + 2, column=4, value=data['标签'])
            self.worksheet.cell(row=i + 2, column=5, value=data['地址'])
            self.worksheet.cell(row=i + 2, column=6, value=data['网址'])
    def save_workboot(self):
        self.workbook.save(f'excel_data/{time.strftime("%Y-%m-%d")}.xlsx')
